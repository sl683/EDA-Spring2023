...
kable(summary_PM25, caption = "Mean Ozone", col.names = c("County", "$\\mu g/m^3$"))
```
...
```{r plot.O3.2018, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Ozone Concentration (2018)", fig.align='left', fig.height= 2.8 }
epa_data %>%
  filter(Year == 2018) %>%
  ggplot(aes(x = factor(Month, labels = month.abb), y = Ozone)) +
  geom_boxplot() +
  labs(
    title ='Ozone Concentration',
    x = 'Month',
    y = expression(paste('Ozone Concentration (',mu,'g)'))
  )
```

```{r plot.O3.2019, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Ozone Concentration (2019)", fig.align='left', fig.height= 2.8 }
epa_data %>%
  filter(Year == 2019) %>%
  ggplot(aes(x = factor(Month, labels = month.abb), y = Ozone)) +
  geom_boxplot() +
...
### Git Repository
https://github.com/sl683/EDA-Spring2023/
