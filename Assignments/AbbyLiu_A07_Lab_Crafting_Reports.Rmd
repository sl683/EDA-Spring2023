...
```{r plot.O3.2018, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Monthly Ozone (2018)", fig.align='left', fig.height= 2.8 }
epa_data %>%
  filter(Year == 2018) %>%
  ...
  labs(
    ...
    x = 'Month',
    y = expression(paste('Concentration (',mu,'g)'))
  )
```

```{r plot.O3.2019, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Monthly Ozone (2019)", fig.align='left', fig.height= 2.8 }
epa_data %>%
  filter(Year == 2019) %>%
  ggplot(aes(x = factor(Month, labels = month.abb), y = Ozone)) +
  geom_boxplot() +
