...
apply(Temp_BR_processed, 2, function(x) any(is.na(x)))
apply(Rainfall_BR_processed, 2, function(x) any(is.na(x)))
...
rainfall_decomposed <- stl(temp_ts, s.window = "periodic")
...
temp_components <- mutate(temp_components, Observed = BR_complete$Temperature_C, Date = BR_complete$Date)
