...
#2:
ozone_boxplot <- ggplot(EPAair, aes(x = as.factor(Year), y = Ozone, fill=as.factor(Year)))+
  geom_boxplot()+
  stat_summary(geom='text',fun=max,vjust=-1,size=4,label=c('a','b'))
print(ozone_boxplot)

#3
ozone_violin <- ggplot(EPAair, aes(x = as.factor(Year), y = Ozone, fill=as.factor(Year)))+
...
pm_o3 <- lm(EPAair$PM2.5 ~ EPAair$Ozone)
summary(pm_o3)
...
pm_o3_plot <- ggplot(EPAair, aes(x=Ozone, y=PM2.5))+
  geom_point()+
...
EPA_AIC <- lm(data = EPAairSub, PM2.5 ~ mean_Lat + mean_Lng + Month + Year + Ozone)
...
model <- lm(data =  EPAairSub, PM2.5 ~ mean_Lat + mean_Lng + Month + Ozone)
summary(model)
```
